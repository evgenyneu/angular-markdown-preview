...
describe 'Directive: iiMdPreview', ->
  ...
  describe 'with textarea-name attribute', ->
    beforeEach ->
      @element = @compileMdPreview("<div class='iiMdPreview' textarea-name='name_for_form' />", @$rootScope)

    it "sets textarea's name attribute", ->
      expect($('textarea', @element).attr('name')).toBe('name_for_form')
